...
md`# HW5 Sample baseline (3pt)`
)}

function _simple(FileAttachment){return(
FileAttachment("sample@2.json").json()
)}
...
function _simple1all(d3,output,invalidation)
{
   // 指定圖表的尺寸。
  const width = 500;
  const height = 400;
...
function _simple4all(d3,output,drag,invalidation)
{
  // 指定圖表的尺寸。
  const width = 500;
  const height = 400;
...
function _simple4(d3,simple,drag,invalidation)
{
  // 指定圖表的尺寸。
  const width = 500;
  const height = 400;
...
function _simple5all(d3,output,drag,invalidation)
{
  // 指定圖表的尺寸。
  const width = 500;
  const height = 400;
...
function _simple5(d3,simple,drag,invalidation)
{
  // 指定圖表的尺寸。
  const width = 500;
  const height = 400;
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["sample@2.json", {url: new URL("../sample.json", import.meta.url), mimeType: "application/json", toString}],
  ...
  main.variable(observer("simple1all")).define("simple1all", ["d3","output","invalidation"], _simple1all);
  main.variable(observer("simple1")).define("simple1", ["d3","simple","invalidation"], _simple1);
  main.variable(observer()).define(["md"], _7);
  main.variable(observer("simple4all")).define("simple4all", ["d3","output","drag","invalidation"], _simple4all);
  main.variable(observer("simple4")).define("simple4", ["d3","simple","drag","invalidation"], _simple4);
  main.variable(observer("drag")).define("drag", ["d3"], _drag);
  main.variable(observer()).define(["md"], _11);
  main.variable(observer("simple5all")).define("simple5all", ["d3","output","drag","invalidation"], _simple5all);
  main.variable(observer("simple5")).define("simple5", ["d3","simple","drag","invalidation"], _simple5);
  return main;
}
